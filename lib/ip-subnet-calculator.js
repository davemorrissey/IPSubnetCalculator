...
var IpSubnetCalculator = {
	...
	calculateExclusions : function (exclusionString) {
		var exclusions = exclusionString.split(/,/);
		var start = 0;
		var result = [];
		for (var i = 0; i < exclusions.length; i++) {
			if (exclusions[i].indexOf('/') > -1) {
				var ip = exclusions[i].substring(0, exclusions[i].indexOf('/'));
				var prefixSize = exclusions[i].substring(exclusions[i].indexOf('/') + 1);
				var range = IpSubnetCalculator.calculateSubnetMask(ip, prefixSize);
				var end = range.ipLow - 1;
				var rangeMasks = IpSubnetCalculator.calculate(IpSubnetCalculator.toString(start), IpSubnetCalculator.toString(end));
				for (var j = 0; j < rangeMasks.length; j++) {
					result.push(rangeMasks[j].ipLowStr + '/' + rangeMasks[j].prefixSize);
				}
				start = range.ipHigh + 1;
			} else {
				var end = IpSubnetCalculator.toDecimal(exclusions[i]) - 1;
				var rangeMasks = IpSubnetCalculator.calculate(IpSubnetCalculator.toString(start), IpSubnetCalculator.toString(end));
				for (var j = 0; j < rangeMasks.length; j++) {
					result.push(rangeMasks[j].ipLowStr + '/' + rangeMasks[j].prefixSize);
				}
				start = IpSubnetCalculator.toDecimal(exclusions[i]) + 1;
			}
		}
		var rangeMasks = IpSubnetCalculator.calculate(IpSubnetCalculator.toString(start), '255.255.255.255');
		for (var j = 0; j < rangeMasks.length; j++) {
			result.push(rangeMasks[j].ipLowStr + '/' + rangeMasks[j].prefixSize);
		}
		return result;
